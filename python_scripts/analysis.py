...
#colors of each channel in the graphs in the "signal" folder
...
def sci(number): 
    ...
    #output: array of strings w/ those numbers in scientific notation
...
def poisson(avg, num): 
    #avg: the average for a poission distribution
    #num: array of whole number values

    #evaluates poisson probability distribution with average avg at each number in num
    return np.array([((avg**n)*np.exp(-avg)/np.math.factorial(n)) for n in num])

def norm(x, avg, stdev, R):
    #x: points at which the normal distribution is evaluated at
    #avg: average of normal distribution
    #stdev: standard deviation of normal distribution
    #R: value between 0 and 1 
    
    #normal distribution multiplied by (1-R) evaluated at points x
    return (1/(stdev*np.sqrt(2*con.pi)))*np.exp((-0.5)*(((x-avg)/stdev)**2))*(1-R)

def tail(x, x0, R): 
    #exponential tail
    #x0 is a parameter on the shape of the exponential tail 
    #R is a number between 0 and 1
    #x is the points at which it is evaluated
...
def erf(x, width, avg, stdev, R): 
    ...
    """
    integral of gaussian function from x-width to x+width
    avg is average of gaussian function 
    stdev is standard deviation of gaussian function 
    (1-R) is the amplitude of the gaussian
...
def exp_int(x, width, x0, R): 
    ...
    """
    integral of exponential tail from x-width to x+width
    
    exponential tail is R/x0*e^(-x/x0)
    """
...
def norm_tail_int(x, width, avg, stdev, R, x0): 
    """
    array with the seperate gaussian integral and exponential integral components of charge distribution
    """
...
def norm_tail_int_seperate(x, width, pe_ch, pe_stdev, zero_ch, zero_stdev, avg_pe_fit, R, x0):
    ...
    #prevents the exponential tail from getting really big to fit the 0PE peak
    y[1:, :, x<0.2]=0
    return y 
...
my_func=interpolate.interp1d(data[0, :, 0], data[led_no], axis=0)

x_arr=np.arange(np.min(data[0, :, 0]), np.max(data[0, :, 0]), 0.1)
led_interp=my_func(x_arr)

if tr_dir=='falling':
    closest_index=np.argmin(np.abs(led_interp[x_arr<-4] - tr_v), axis=0)
    print(np.min(closest_index))
else: 
    closest_index=np.argmin(np.abs(led_interp[x_arr>4] - tr_v), axis=0)
    closest_index=closest_index+led_interp[x_arr<=4].shape[0]
...
c=np.argmin(led_duration)
plt.figure(); 
plt.plot(x_arr, led_interp[:, c], linestyle='None', marker='.')
plt.title('Trace of smallest duration')
...
title0='Input voltage ' + v_set + ' V and width ' + wid_set + ' ns'
...
#v=sci(np.array([v_set]).astype(float))
#w=sci(np.array([wid_set]).astype(float))

v=v_set
w=wid_set

try: 
    os.mkdir(analysis_path + '/led_peak')
except: 
    pass

try: 
    os.mkdir(analysis_path + '/led_duration')
except: 
    pass

try: 
    os.mkdir(analysis_path + '/led_peak_hist')
except: 
    pass

try: 
    os.mkdir(analysis_path + '/led_duration_hist')
except: 
    pass

plt.figure()
plt.plot(np.max(data[led_no], axis=0), linestyle='None', marker='.')
plt.xlabel('Waveform Number')
plt.ylabel('Peak (in mV)')
plt.title('Input Voltage ' + v + ' V and Input width ' + w + ' ns')

plt.savefig(analysis_path + '/led_peak/' + file_name + '.png')

plt.figure()
plt.plot(led_duration, linestyle='None', marker='.')
plt.title('Input Voltage '+ v + ' V and Input Width ' + w + ' ns')
plt.xlabel('Waveform Number')
plt.ylabel('Duration (in ns)')
plt.savefig(analysis_path + '/led_duration/' + file_name + '.png')

plt.figure()
plt.hist(np.max(data[led_no], axis=0), bins=100)
plt.xlabel('Peak Voltage (mV)')
plt.ylabel('Counts')
plt.title('Input Voltage '+ v + ' V and Input Width ' + w + ' ns')
plt.savefig(analysis_path + '/led_peak_hist/' + file_name + '.png')

plt.figure()
plt.hist(led_duration, bins=100)
plt.xlabel('LED Duration (ns)')
plt.ylabel('Counts')
plt.title('Input Voltage '+ v + ' V and Input Width ' + w + ' ns')
plt.savefig(analysis_path + '/led_duration_hist/' + file_name + '.png')
