...
data_dir='/home/labwork/autocontrol/output_files/azimuth3_long_term_run10/raw_data'
check_mode=0
pmt_ch = 0
ch_enable=np.array([1, 0, 1, 0])
#setting=sys.argv[8]
analysis_path='/home/labwork/autocontrol/output_files/azimuth3_long_term_run10/processed_data'
...
for e in range(len(data_list)):
    ...
    lower_bound=np.array([0, 0, -np.inf, -np.inf, -np.inf, 0.05, 0.1, 0])
    upper_bound=np.array([np.inf, np.inf, 0.5, np.inf, np.inf, 0.9, 10, np.inf])
    param0=np.array([pe1, 0.9, pe0, 0.12, avg_pe_guess, 0.2, 2.3, h_guess])
    ...
    if np.all(combined_condition):
        try:    
            print('Fitting...')
            #popt, pcov = op.c  urve_fit(norm_tail_sum, bin_center, count, p0=param0, bounds=(lower_bound, upper_bound), maxfev=50000)
            popt, pcov = op.c  urve_fit(norm_tail_int_sum, bin_center, count, p0=param0, bounds=(lower_bound, upper_bound), args = (width), maxfev=50000)
        except:
            print('Failed.')
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig(analysis_path + '/' + charge_dist_folder + '/' + data_loc[:-4] + '.png')
