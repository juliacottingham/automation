...
for e in range(bin_final):
    ...
    closest_index=np.argmin(np.abs(data[2][data[0, :, 0]<-20] + 70), axis=0)
    ...
    led_charge=integral.trapezoid((-1)*led_v_integral, signal[0], axis=0)*unit_factor
    avg_led_charge=np.average(led_charge)
    med_led_charge=np.median(led_charge)
    std_led_charge=np.std(led_charge)
    
    avg_led_peak= np.average(np.min(data[2], axis=0))
   
    diff_arr=np.abs(data[2]+70) 
