...
for n in range(led_v_array.shape[1]-3): 
    plotting_function1(led_v_array[:, n+1], titles[n], 'Time', ylabels[n])


plt.figure()
plt.plot(led_v_array2[:, 4], led_v_array2[:, 5], linestyle='None', marker='o')
plt.xlabel('CH C Peak Voltage (mV)')
plt.ylabel('CH C Time above 60 mV (ns)')
plt.title('Relationship between Channel C Voltage and Duration')
...
fig, ax = plt.subplots()
plt.errorbar(led_v_array2[condition, 5], avg_pe[condition, 0], yerr=avg_pe[condition, 1], linestyle='None', marker='.', label='Estimated from Occupancy')
plt.errorbar(led_v_array2[~condition, 5], avg_pe[~condition, 0], yerr=avg_pe[~condition, 1], linestyle='None', marker='.', label='Estimated from Charge Distribution')
plt.xlabel('Measured duration above 60 mV (ns)')
plt.ylabel('Avg PE')
plt.title('Light level with measured duration')
#ax.set_xscale("log")
ax.set_yscale("log")
plt.legend()
...
x_data0=led_v_array2[:, 4]
y_data0=led_v_array2[:, 5]
z_data0=avg_pe[:, 0]

ax.scatter3D(x_data0, y_data0, z_data0)
